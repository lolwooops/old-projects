...
WCSts = ts(datats2[,2],start=c(2009,1),end=c(2018,8),frequency=12)
n.end=2014+11/12
mat0 <- matrix(rep(NA,108),36,3)
for(i in 1:36){
  tmp0 <- 2009
  tmp1 <- n.end+(i-1)*1/12
  tmp <- window(y1,tmp0,tmp1)
  mat0[i,1] <- tmp1-tmp0+1 # obs for estimation
  mat0[i,2] <- window(y1,tmp1+1/12,tmp1+1/12) # actual
  fit1 <- VAR(datats2[,2:5], p=2, type="const")
  mat0[i,3] <- forecast(fit1, h=1)$mean
}

n.end=2014+11/12
mat0 <- matrix(rep(NA,144),48,3)
for(i in 1:48){
  tmp0 <- 2009
  tmp1 <- n.end+(i-1)*1/12
  tmp <- window(WCSts,tmp0,tmp1)
  mat0[i,1] <- tmp1-tmp0+1/12 # obs for estimation
  mat0[i,2] <- window(WCSts,tmp1+1/12,tmp1+1/12) # actual
  fit1 <- VAR(datats2[,2:5], p=2, type="const")
  mat0[i,3] <- forecast(fit1, h=1)$mean
}
